...
const MONGO_URI = process.env.MONGO_URI || 'mongodb://127.0.0.1:27017/saas_notes';
...
mongoose.connection.once('open', () => {
  if (!process.argv.includes('--seed')) {
    ensureDefaultData().catch((err) => console.error('Ensure default data error', err));
  }
});
...
app.post('/auth/login', async (req, res) => {
  ...
  res.json({ token, role: user.role, tenant: tenant ? { slug: tenant.slug, plan: tenant.plan, name: tenant.name } : undefined });
});
